...
class FormulaFactory(object):
  ...
  def parseformula(self, formula):
    ...
    if not pos.finished():
      ...
      whole.add_word(TaggedBit().constant(formula, 'span class="unknown"'))
    return whole
...
class LimitsProcessor(MathsProcessor):
  ...
  def limitsahead(self, contents, index):
    ...
    contents[index + 1].add_word(contents[index].clone())
...
class MacroFunction(CommandBit):
  ...
  def parsenumbers(self, pos, remaining):
    ...
    for digit in number.original:
      ...
      value.add_word(FormulaConstant(digit))
      value.type = number
      self.values.append(value)
    return None
